...
func TestHandler_PolicyAddEventHandler(t *testing.T) {
	...
	assert.Equal(t, store.GetPrefixPolicies(getEndpoint(getDefaultService(), policy.GET, "/")), getRoutePolicy(key))
	assert.Equal(t, store.GetPrefixPolicies(getEndpoint(getDefaultService(), policy.GET, "/path")), getRoutePolicy(key))
	assert.Equal(t, store.GetPrefixPolicies(getEndpoint(getDefaultService(), policy.GET,"/revision")), getRoutePolicy(key))
	targets = []v1.TargetElement{
	...
	assert.Equal(t, store.GetPolicies(getEndpoint(getDefaultService(), policy.GET, "/paths")), policy.NewRoutePolicy())
	assert.Equal(t, store.GetPolicies(getEndpoint(getDefaultService(), policy.GET, "/userpath")), getRoutePolicy(key))
	assert.Equal(t, store.GetPrefixPolicies(getEndpoint(getDefaultService(), policy.GET, "/paths")), getRoutePolicy(key))
	assert.Equal(t, store.GetPrefixPolicies(getEndpoint(getDefaultService(), policy.GET, "/paths/path")), getRoutePolicy(key))
	assert.Equal(t, store.GetPrefixPolicies(getEndpoint(getDefaultService(), policy.GET, "/userpath")), policy.NewRoutePolicy())
	targets = []v1.TargetElement{
		getTargetElements(service, getPathConfigs(getPathConfig("/path", "/", "GET", getPathPolicy()))),
	}
	handler = GetAddEventHandler(policyGenerator(targets), store, fake.NewSimpleClientset())
